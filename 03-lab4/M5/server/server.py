...
class YorkshireEncryption:
    ...
    def pad(cls, msg: bytes):
        ...
        print("PADDED SECRET", (bit_pading + msg)[:16])
...
class BackupServer(CommandServer):
    ...
    def place_flag(self):
        ...
        """
        self.secret_file = (
            f"{secrets.randbelow(10000)}: don't forget that this is your secret AC login code.".encode()
            + b" " * 32
        )

        print(self.secret_file[:16].decode())
        file_ctxt = self.encrypt("admin", self.secret_file)
        ctxt_hash = file_ctxt[-16:]
        self.db[ctxt_hash] = file_ctxt
        print(
            "CTXT TO FIND",
            self.db[ctxt_hash].hex(),
            len(self.db[ctxt_hash]),
            self.secret_file.hex(),
            len(self.secret_file),
        )
    ...
    def handle_list(self, msg):
        ...
        try:
            self.send_message(
                {"result": list(ctxt_hash.hex() for ctxt_hash in self.db.keys())}
            )
        except (KeyError, ValueError, TypeError):
            self.send_message({"error": f"Invalid parameters."})
    ...
    def handle_check(self, msg):
        ...
        try:
            ...
            # print("START", self.db[ctxt_hash][:16].hex(), ctxt_start.hex())
            # print("START", self.db[ctxt_hash][:16], ctxt_start)
