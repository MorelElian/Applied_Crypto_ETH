...
class CarpetServer(CommandServer):
    ...
    def set_response_key_handler(self, msg):
        ...
        try:
            self.key_enc = ElGamal.construct(
                (int(msg["p"]), int(msg["g"]), int(msg["y"]), 4)
            )
    ...
    def encrypted_command_handler(self, msg):
        ...
        try:
            ...
            if int.from_bytes(enc_command_c1, "big") % int(self.key_dec.p) == 1:
                raise ValueError("Nice try!")

            command = ElGamalImpl.decrypt(self.key_dec, enc_command_c1, enc_command_c2)
        ...
        print(ElGamalImpl.decrypt(self.key_enc, c1, c2))
