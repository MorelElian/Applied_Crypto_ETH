...
class RSADecryptionOracle(CommandServer):
    ...
    def encrypt(self, ptxt: str):
        ...
        print("PTXT", ptxt, self.key.e, self.key.n)
        ctxt = pow(ptxt_int, self.key.e, self.key.n)
        ctxt_bis = pow(2 * ptxt_int, self.key.e, self.key.n)
        test = hex(
            (pow(2, self.key.e) * pow(ptxt_int, self.key.e, self.key.n)) % self.key.n
        )
        print("TEST", test)
        print("CTXT BIS : ", hex(ctxt_bis))
