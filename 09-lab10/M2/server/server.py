...
class BeaconServer(CommandServer):
    ...
    def handle_new_planet(self, _):
        try:
            ...
            fingerprint = SHA256.new(data=key_serialized).digest()[
    ...
    def handle_signal(self, msg):
        try:
            ...
            fingerprint = SHA256.new(data=key.public_key().export_key()).digest()[
                : self.fingerprint_len
            ]
            if a == fingerprint:
                print("SAME FINGERPRINT WE ARE DOOMED")
            # Only the original registrar of a planet can change its habitable status.
            # Use signatures to enforce this!
            print("BEFORE")
            verifier.verify(
                "expedition started".encode() + fingerprint, self.planet_db[name]
            )
            print("AFTER")
