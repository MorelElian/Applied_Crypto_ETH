...
class WickedServer(CommandServer):
    def __init__(self, flag, *args, **kwargs):
        ...
        self.the_table = {
            name: {"key": genkey(), "inbox": list()} for name in THE_HIGH_TABLE
        }
        ...
        print(
            "COMMON SECRET",
            self.common_secret,
            int.from_bytes(self.common_secret.encode()),
            pow(int.from_bytes(self.common_secret.encode()), 17),
        )
    ...
    def handle_pubkey(self, msg):
        ...
        for name, data in self.the_table.items():
            ...
            for message in to_send:
                ...
                # if message == self.common_secret:
                #    print(key["e"], enc, pow(message_int, key["e"]))
                out[name].append(enc)
    ...
    def get_pubkey(self, msg):
        self.send_message(
            {"error": "You are not allowed to learn the data of The High Table"}
        )
    ...
    def vote(self, msg):
        ...
        for name, data in self.the_table.items():
            ...
            for expected_msg in MESSAGES:
                ...
                if dec != expected_msg:
                    self.send_message(
                        {"error": "Tampering detected. You shall pay for this"}
                    )
    ...
    def handle_adjudicator(self, msg):
        ...
        try:
            ...
            if len(votes) != len(THE_HIGH_TABLE) or not all(
                name in votes for name in THE_HIGH_TABLE
            ):
                print(len(votes))
            ...
            if count_favour != 0:
                self.send_message(
                    {
                        "message": "By thine own hand, your membership to the Continental has been revoked. "
                    }
                )
            else:
            ...
            self.send_message(
                {"error": "Decoding has failed. You have sealed your fate."}
            )
            self.close_connection()
